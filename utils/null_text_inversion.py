...
class NullInversion:
    ...
    def get_noise_pred(self, latents, t, is_forward=True, context=None):
        latents_input = jt.concat([latents] * 2)
        if context is None:
    ...
    def latent2image(self, latents, return_type='np'):
        ...
        if return_type == 'np':
            ...
            image0 = jt.misc.cpu(image)
            image = image0.permute(0, 2, 3, 1).numpy()[0]
    ...
    def image2latent(self, image):
        with jt.no_grad():
            ...
            if type(image) is jt.Var and image.dim() == 4:
                ...
                image = jt.array(image).float32() / 127.5 - 1
    ...
    def ddim_loop(self, latent):
        ...
        for i in range(NUM_DDIM_STEPS):
            t = self.model.scheduler.timesteps[len(self.model.scheduler.timesteps) - i - 1]  # 这里必须要用DDIM规划期
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        bar = tqdm(total=num_inner_steps * NUM_DDIM_STEPS,position=0,leave=True)
        for i in range(NUM_DDIM_STEPS):
            # null_optim日志开始
            logger.log_gpu(step=i, gpu_mem_mb=pynvml.nvmlDeviceGetMemoryInfo(pynvml.nvmlDeviceGetHandleByIndex(1)).used / 1024 ** 2)
            uncond_embeddings = uncond_embeddings.clone().detach()
            uncond_embeddings.requires_grad = True
            optimizer = jt.optim.Adam([uncond_embeddings], lr=1e-2 * (1. - i / 100.))
            latent_prev = latents[len(latents) - i - 2]
            t = self.model.scheduler.timesteps[i]
            with jt.no_grad():
                noise_pred_cond = self.get_noise_pred_single(latent_cur, t, cond_embeddings)
            for j in range(num_inner_steps):
                tic_null = time.perf_counter()  # null_optim计时开始
                noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, uncond_embeddings)
                noise_pred = noise_pred_uncond + GUIDANCE_SCALE_TRAIN * (noise_pred_cond - noise_pred_uncond)
                latents_prev_rec = self.prev_step(noise_pred, t, latent_cur)
                loss = jt.nn.mse_loss(latents_prev_rec, latent_prev)
                optimizer.zero_grad()
                optimizer.backward(loss)
                optimizer.step()
                loss_item = loss.numpy().item()
                toc_null = time.perf_counter()
                dt = toc_null - tic_null  # null_optim计时结束
                logger.log_null_optim(step=i, iter=j, loss=loss_item, timestamp=dt)  # null_optim日志结束
                bar.update()
            ...
            with jt.no_grad():
                context = jt.concat([uncond_embeddings, cond_embeddings])
