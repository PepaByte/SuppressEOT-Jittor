...
def latent2image(vae, latents):
    ...
    image = image0.permute(0, 2, 3, 1).numpy()
...
def text2image_ldm(
    ...
    context = jt.concat([uncond_embeddings, text_embeddings])
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        to_out = self.to_out
        if type(to_out) is jt.nn.ModuleList:
...
def update_alpha_time_word(alpha, bounds: Union[float, Tuple[float, float]], prompt_ind: int,
                           word_inds: Optional[jt.Var] = None):
    if type(bounds) is float:
        bounds = 0, bounds
    start, end = int(bounds[0] * alpha.shape[0]), int(bounds[1] * alpha.shape[0])
    if word_inds is None:
        word_inds = jt.arange(alpha.shape[2])
...
def get_time_words_attention_alpha(prompts, num_steps,
    ...
    alpha_time_words = jt.zeros(num_steps + 1, len(prompts) - 1, max_num_words)
