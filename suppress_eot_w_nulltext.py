...
#import torch
import jittor as jt
from JDiffusion import StableDiffusionPipeline#from diffusers import StableDiffusionPipeline, DDIMScheduler
...
from utils.log_csv import logger
import time
import pynvml
...
def text2image_ldm_stable(
    ...
    latents = jt.concat([_latent, _latent_erase])
    ...
    for i, t in enumerate(pbar):
        if uncond_embeddings_ is None:
            context = jt.concat([uncond_embeddings[i].expand(*text_embeddings.shape), text_embeddings])
            if LOW_RESOURCE:
                context = (uncond_embeddings[i].expand(*text_embeddings.shape), text_embeddings)
        else:
            context = jt.concat([uncond_embeddings_, text_embeddings])
            if LOW_RESOURCE:
        ...
        if controller.MAX_STEP_TO_ERASE > controller.i >= controller.cross_retain_steps and not (controller.text_embeddings_erase == text_embeddings).all() and \
            ...
            while iter > 0:
                with jt.enable_grad():
                    ...
                    controller.attention_store = {}
                    # conditional branch
                    text_embeddings_erase = controller.text_embeddings_erase.clone().detach()
                    text_embeddings_erase.requires_grad = True
                    ...
                    if loss != .0:
                        pbar.set_postfix({'loss': loss if isinstance(loss, float) else loss.numpy().item()})
        ...
        contexts = [jt.concat([context[0], context_erase[0]]), jt.concat([context[1], context_erase[1]])]
...
def update_context(context: jt.Var, loss: jt.Var, scale: int, factor: float) -> jt.Var:
    """
    ...
    :return:
    """
    grad_cond = jt.grad(loss=loss, targets=[context], retain_graph=False)[0]
...
def load_model(sd_version):
    ...
    ldm_stable = StableDiffusionPipeline.from_pretrained(stable_diffusion_version, scheduler=scheduler)#.to(device) # jittor.to(Var,device)只转移Var？
    # try:
        ...
        for cross_retain_steps in args.cross_retain_steps:
            for alpha in args.alpha:
                ...
                controller = AttentionStore(token_indices, alpha, args.method, cross_retain_steps, n, args.iter_each_step, args.max_step_to_erase,
                                            lambda_retain=args.lambda_retain, lambda_erase=args.lambda_erase, lambda_self_retain=args.lambda_self_retain, lambda_self_erase=args.lambda_self_erase)
                tau = round(1.0 - cross_retain_steps, 3)
                image_inv, x_t = run_and_display(stable, [prompt], controller, latent=None, uncond_embeddings=None, verbose=False)
